...
from mlp_utils.layers import FeedForward, FastFeedForward, MLP, PathWeightedFFF, SwitchFFN, NGPT  # noqa: E402
...
def test_feedforward_forward_signature_and_behavior(module_ctor, ctor_kwargs, expects_tuple):
    torch.manual_seed(0)
    ...
    assert non_self_params, f"{module_ctor.__name__}.forward should accept at least one argument besides self"
    assert (
        non_self_params[0].name == "x"
    ...
    if expects_tuple:
        ...
        assert y.shape == x.shape, f"Output shape {y.shape} must match input shape {x.shape}"
        assert isinstance(aux, torch.Tensor) and aux.ndim == 0, "Auxiliary loss must be a 0-D tensor"
    else:
        assert isinstance(out, torch.Tensor), "Output must be a tensor"
        assert out.shape == x.shape, f"Output shape {out.shape} must match input shape {x.shape}"
