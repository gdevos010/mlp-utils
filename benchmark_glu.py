...
class GLUBenchmark:
    ...
    def run_benchmarks(self) -> Dict[str, Dict[str, Dict[str, float]]]:
        ...
        for config in self.test_configs:
            config_name = f"B{config['batch_size']}_I{config['dim_in']}_O{config['dim_out']}"
            ...
            for variant_name, variant_class in all_variants.items():
                ...
                try:
                    benchmark_result = self.benchmark_variant(variant_name, variant_class, config)
    ...
    def print_results_table(self, results: Dict[str, Dict[str, Dict[str, float]]]) -> None:
        ...
        for config_name, config_results in results.items():
            ...
            # Prepare table data
            headers = ["Variant", "Forward (ms)", "Backward (ms)", "Total (ms)", "Fwd Mem (MB)", "Bwd Mem (MB)", "Parameters"]
            table_data = []
            ...
            for variant in sorted_variants:
                ...
                if "error" in stats:
                    table_data.append([variant, "ERROR", stats['error'], "", "", "", ""])
                else:
                    table_data.append([
                        variant,
    ...
    def compare_glu_vs_mglu(self, results: Dict[str, Dict[str, Dict[str, float]]]) -> None:
        ...
        for config_name, config_results in results.items():
            ...
            headers = ["Variant Pair", "Time Ratio (MGLU/GLU)", "Memory Ratio (MGLU/GLU)", "Param Ratio (MGLU/GLU)", "Performance"]
            table_data = []
            ...
            for glu_name, mglu_name in glu_mglu_pairs:
                if glu_name in config_results and mglu_name in config_results:
                    ...
                    if "error" not in glu_stats and "error" not in mglu_stats:
                        time_ratio = mglu_stats["total_time_ms"] / glu_stats["total_time_ms"]
                        memory_ratio = mglu_stats["backward_memory_mb"] / glu_stats["backward_memory_mb"]
...
def main():
    ...
    print("- In comparison ratios: < 1.0 indicates MGLU is better, > 1.0 indicates GLU is better")
    print("- Performance indicators: ✓ (both time and memory better), ✗ (both worse), ~ (mixed results)")
